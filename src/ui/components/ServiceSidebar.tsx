...
export function ServiceSidebar({
  ...
  tracker,
}: ServiceSidebarProps): React.ReactElement {
  ...
  return (
    <Box flexDirection="column" paddingX={1} paddingY={1}>
          ...
          return (
            <Box key={service.name}>
              <Text color={theme.muted.fg}>
                {idx + 1}. {statusIndicator(service.status)}{' '}
              </Text>
              <Text color={healthColor(health)}>● </Text>
              <Text color={hidden ? theme.muted.fg : service.color} bold={isSolo}>
      ...
      <Box flexDirection="column">
        {errorCount > 0 && (
          <Text color={theme.levels.error.fg}>
            {errorCount} error{errorCount > 1 ? 's' : ''}
          </Text>
        )}
        {warningCount > 0 && (
          <Text color={theme.levels.warn.fg}>
