export function parseTimestamp(value: string | number | undefined): Date | undefined {
  ...
  const accessLogMatch = trimmed.match(
...
export function extractTimestampFromPrefix(line: string): { timestamp?: Date; rest: string } {
  ...
  if (isoMatch) {
    return {
      timestamp: parseTimestamp(isoMatch[1]),
      rest: line.slice(isoMatch[0].length).trimStart(),
    };
  }
  ...
  if (dateTimeMatch) {
    return {
      timestamp: parseTimestamp(dateTimeMatch[1].replace(',', '.')),
      rest: line.slice(dateTimeMatch[0].length).trimStart(),
    };
  }
  ...
  if (bracketTimeMatch) {
    return {
      timestamp: parseTimestamp(bracketTimeMatch[1]),
      rest: line.slice(bracketTimeMatch[0].length),
    };
  }
  ...
  if (simpleTimeMatch) {
    return {
      ...
      rest: line.slice(simpleTimeMatch[0].length),
    };
  }
  ...
  if (unixMsMatch) {
    return {
      timestamp: parseTimestamp(Number.parseInt(unixMsMatch[1], 10)),
      rest: line.slice(unixMsMatch[0].length).trimStart(),
    };
  }
  ...
  if (unixSecMatch) {
    return {
      timestamp: parseTimestamp(Number.parseInt(unixSecMatch[1], 10)),
      rest: line.slice(unixSecMatch[0].length).trimStart(),
    };
  }
